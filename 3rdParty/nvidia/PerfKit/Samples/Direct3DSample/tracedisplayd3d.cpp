...
void CD3DTraceDisplay::Background()
{
	if (_draw_back)
	{
		...
		DWORD color = D3DCOLOR_RGBA(int(255.0f*_background_color[0]),int(255.0f*_background_color[1]),int(255.0f*_background_color[2]),int(255.0f*_background_color[3]));
...
void CD3DTraceDisplay::Text(const Entries::iterator &it, const float count)
{
	if (_draw_text)
	{
		...
		DWORD color = D3DCOLOR_RGBA(int(255.0f * it->r), int(255.0f * it->g), int(255.0f * it->b), int(255.0f * it->a));
		
		_pd3dDevice->GetViewport(&view);
		
		if(_bDisplayMinMax)
			_sntprintf(msg, 256, _T("%S: %.2f [%.2f %.2f]"), it->trace->name().c_str(), it->trace->last(), it->trace->min(), it->trace->max());
		else
			_sntprintf(msg, 256, _T("%S: %.2f"),  it->trace->name().c_str(), it->trace->last(), it->trace->min(), it->trace->max());
...
void CD3DTraceDisplay::DisplayLineStream()
{
	...
	for(Entries::iterator it = _entries.begin(); it != _entries.end(); ++it)
	{
		unsigned int v = 0;
		DWORD color = D3DCOLOR_RGBA(int(255.0f * it->r), int(255.0f * it->g), int(255.0f * it->b), int(255.0f * it->a));
		float bounds = it->trace->max() - it->trace->min();
		for (size_t i = 0; i < it->trace->size(); ++i)
		{
			_verts[v].x = (_left + (_width * (float(i) / float(it->trace->capacity() - 1)))) * view.Width;
			_verts[v].y = (fLocalBottom - (fLocalHeight * (((*(it->trace))(i) - it->trace->min()) / bounds))) * view.Height;
